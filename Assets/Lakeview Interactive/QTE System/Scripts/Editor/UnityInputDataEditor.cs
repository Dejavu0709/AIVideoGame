...
namespace QTESystem
{
    ...
    public class UnityInputDataEditor : InputDataEditor
    {
        ...
        public override void OnEnable()
        {
            ...
            mouseButtons = new ReorderableList(serializedObject, serializedObject.FindProperty("mouseButtons"), true, true, true, true);

            mouseButtons.drawElementCallback =
                (Rect rect, int index, bool isActive, bool isFocused) =>
                {
                    var element = mouseButtons.serializedProperty.GetArrayElementAtIndex(index);

                    DrawElement(rect, element);


                };
            mouseButtons.drawHeaderCallback = (Rect rect) => {
                EditorGUI.LabelField(rect, "Mouse Buttons");
            };
        ...
        private void ModifyControllerOptions()
        {
            ...
            {
                if (gamepad[button] != null && (gamepad[button]).isPressed)
                {
        ...
        protected void DrawMouseOptions(UnityEngine.Object target)
        {
            var inputData = (UnityInputData)target;

            mouseButtons.DoLayoutList();
