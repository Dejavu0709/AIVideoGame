...
using BestHTTP.Addons.TLSSecurity.Examples.CSV;
...
namespace BestHTTP.Addons.TLSSecurity.Examples.BatchedTest
{
    public struct Batch
    {
    ...
    public class BatchedTestEntry : MonoBehaviour
    {
        ...
        {
#if !UNITY_WEBGL || UNITY_EDITOR
            // Initialize the TLS Security Addon
            BestHTTP.Addons.TLSSecurity.TLSSecurity.Setup();
#endif
...
#endif
                request.ConnectTimeout = TimeSpan.FromMinutes(1);
                request.Timeout = TimeSpan.FromMinutes(10);

#if !UNITY_WEBGL || UNITY_EDITOR
                request.Tag = new TLSTagEx { RemoveALPNExtension = true, Tag = kvl };
#else
                request.Tag = kvl;
#endif
...
#if !UNITY_WEBGL || UNITY_EDITOR
            KeyValueLine kvl = (req.Tag as TagEx)?.Tag as KeyValueLine;
#else
