...
{
    public class TLSTagEx : TagEx
    {
        public bool RemoveALPNExtension;
    }

    public sealed class SecureTlsClient : AbstractTls13Client
    {
        private TLSTagEx _tag = null;
        private Uri _currentUri = null;

        public SecureTlsClient(HTTPRequest request, List<ServerName> hostNames, List<ProtocolName> clientSupportedProtocols)
            : base(request, hostNames, clientSupportedProtocols, new FastTlsCrypto(new SecureRandom()))
        {
            this._tag = this._request.Tag as TLSTagEx;
            this._currentUri = this._request.CurrentUri;
        }
        ...
        byte SignatureAlgorithmFromPrivateKey(SecureProtocol.Org.BouncyCastle.Crypto.AsymmetricKeyParameter key)
        {
            if (key is SecureProtocol.Org.BouncyCastle.Crypto.Parameters.RsaKeyParameters)
            {
                return (byte)SignatureAlgorithm.rsa;
            }
            else if (key is SecureProtocol.Org.BouncyCastle.Crypto.Parameters.DsaPrivateKeyParameters)
            {
                return (byte)SignatureAlgorithm.dsa;
            }
            else if (key is SecureProtocol.Org.BouncyCastle.Crypto.Parameters.ECPrivateKeyParameters)
            {
        ...
        public override void NotifyServerCertificate(TlsServerCertificate serverCertificate)
        {
            ...
            {
                if (HTTPManager.Logger.Level == Logger.Loglevels.All)
                    HTTPManager.Logger.Exception(nameof(SecureTlsClient), $"{nameof(TlsAuthentication.NotifyServerCertificate)}", ex, this.Context);
