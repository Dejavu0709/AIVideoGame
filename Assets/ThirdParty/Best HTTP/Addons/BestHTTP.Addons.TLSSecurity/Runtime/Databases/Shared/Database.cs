...
namespace BestHTTP.Addons.TLSSecurity.Databases.Shared
{
        ...
        where MetadataServiceType : MetadataService<MetadataType, ContentType>
    {
            ...
            MetadataServiceType metadataService)
        {
            ...
            this.DiskManager = new DiskManager<ContentType>(
                HTTPManager.IOService.CreateFileStream(this.DatabaseFileName, PlatformSupport.FileSystem.FileStreamModes.OpenReadWrite),
                HTTPManager.IOService.CreateFileStream(this.DatabaseFreeListFileName, PlatformSupport.FileSystem.FileStreamModes.OpenReadWrite),
                diskContentParser,
                options.DiskManager);

            using (var fileStream = HTTPManager.IOService.CreateFileStream(this.MetadataFileName, PlatformSupport.FileSystem.FileStreamModes.OpenReadWrite))
        ...
        public void Save()
        {
            ...
            using (new WriteLock(this.rwlock))
            {
                using(var fileStream = HTTPManager.IOService.CreateFileStream(this.MetadataFileName, PlatformSupport.FileSystem.FileStreamModes.Create))
                    using (var stream = new BufferedStream(fileStream))
                        this.MetadataService.SaveTo(stream);

                if (this.Options.UseHashFile)
                {
                    using (var hashStream = HTTPManager.IOService.CreateFileStream(this.HashFileName, PlatformSupport.FileSystem.FileStreamModes.Create))
                    {
