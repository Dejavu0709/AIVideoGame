...
namespace BestHTTP.Addons.TLSSecurity.Install
{
    public sealed class UnpackDatabaseScript : MonoBehaviour
    {
        ...
        IEnumerator LoadAndWrite(string targetFolder, string resourceName, string extension)
        {
            ...
            using (var outStream = HTTPManager.IOService.CreateFileStream(fileName, PlatformSupport.FileSystem.FileStreamModes.Create))
        ...
        bool CompareAssets(TextAsset asset, string fileName)
        {
            ...
            try
            {
                ...
                {
                    using (var outStream = HTTPManager.IOService.CreateFileStream(fileName, PlatformSupport.FileSystem.FileStreamModes.OpenRead))
                    {
