...
namespace BestHTTP.Addons.TLSSecurity.OCSP
{
    public static class OCSPCache
    {
        ...
        public static Status GetCertificateStatus(List<X509Certificate> chain, AuthorityInformationAccess aia, BestHTTP.Logger.LoggingContext loggingContext)
        {
            if (chain.Count < 2)
        ...
        private static List<AccessDescription> CreateRequests(List<X509Certificate> chain, AuthorityInformationAccess aia, bool setTimeouts, Action<HTTPRequest, HTTPResponse, List<AccessDescription>> callback, BestHTTP.Logger.LoggingContext loggingContext)
        {
            ...
            for (int i = 0; i < ocspEndpoints.Count; ++i)
            {
                ...
                if (setTimeouts)
                {
                    request.ConnectTimeout = SecurityOptions.OCSP.OCSPCache.HTTPRequestOptions.ConnectTimeout;
                    request.Timeout = SecurityOptions.OCSP.OCSPCache.HTTPRequestOptions.Timeout;
                }
        ...
        private static void SendRefreshRequest(byte[] hash, List<X509Certificate> chain, AuthorityInformationAccess aia, BestHTTP.Logger.LoggingContext loggingContext)
        {
                        ...
                        if (resp.IsSuccess)
                        {
                            if (resp.GetFirstHeaderValue("Content-Type") == "application/ocsp-response")
                            {
                                ...
                                try
                                {
                                    ...
                                    var CertificateStatus = new BestHTTP.SecureProtocol.Org.BouncyCastle.Tls.CertificateStatus(CertificateStatusType.ocsp, response);
        ...
        private static (Status, SingleResponse) SendBlockingRequest(List<X509Certificate> chain, AuthorityInformationAccess aia, BestHTTP.Logger.LoggingContext loggingContext)
        {
            HTTPManager.Logger.Information(nameof(OCSPCache), $"{nameof(SendBlockingRequest)} - AuthorityInformationAccess: {aia}", loggingContext);

            var queue = new ConcurrentQueue<BestHTTP.SecureProtocol.Org.BouncyCastle.Tls.CertificateStatus>();
                    ...
                    case HTTPRequestStates.Finished:
                        if (resp.IsSuccess)
                        {
                            if (resp.GetFirstHeaderValue("Content-Type") == "application/ocsp-response")
                            {
                                ...
                                queue.Enqueue(new BestHTTP.SecureProtocol.Org.BouncyCastle.Tls.CertificateStatus(CertificateStatusType.ocsp, response));
                            }
                            else
