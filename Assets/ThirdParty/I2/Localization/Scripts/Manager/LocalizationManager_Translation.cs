...
namespace I2.Loc
{
    public static partial class LocalizationManager
    {
        ...
        public static string GetTranslationWithParameters(string Term, params object[] param)
        {
            string Translation = null;
            Translation = GetTranslation(Term);
            if(string.IsNullOrEmpty(Translation))
                return Translation;
            bool zero_base = Translation.Contains("{0", System.StringComparison.Ordinal);
			
			if (zero_base)
			{
				try
				{
					Translation = string.Format(Translation, param);
					return Translation;
				}
				catch (System.Exception e)
				{
					Debug.Log("GetFormattedWithPara: " + e.Message);
				}
			}
