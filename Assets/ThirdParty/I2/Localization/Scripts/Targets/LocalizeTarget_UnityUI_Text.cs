...
namespace I2.Loc
{
    ...
    public class LocalizeTarget_UnityUI_Text : LocalizeTarget<Text>
	{
		...
		{
            var localize = mTarget.GetComponent<Localize>();
			if (mTarget && localize)
			{
				primaryTerm = localize.Term;
				secondaryTerm =string.IsNullOrEmpty(localize.SecondaryTerm)? "1fonts" : localize.SecondaryTerm;
                if (!string.IsNullOrEmpty(primaryTerm))
					return;
			}			
            primaryTerm = mTarget ? mTarget.text : null;
			//写死
			secondaryTerm = "1fonts";
           // secondaryTerm = mTarget.font!=null ? mTarget.font.name : ;
		}
